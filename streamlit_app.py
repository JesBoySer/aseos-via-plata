                ...
                for fila in range(2):
                    ...
                    if fila < len(ocupados):
                        ...
                        if cols[5].button("Salida", key=f"fin_{key}"):
                            ...
                            conn.commit()
                            df_sql = pd.read_sql_query("SELECT * FROM visitas", conn)
                            conn.close()
                            save_to_github(df_sql)
                        ...
                        if st.session_state.editar[key]:
                            # Selectboxes con placeholder
                            curso = st.selectbox("Curso", sorted(df_alumnos["Curso"].unique()), key=f"curso_{key}", index=-1)
                            alumno = st.selectbox("Alumno", df_alumnos[df_alumnos["Curso"]==curso]["Nombre"], key=f"alumno_{key}", index=-1)
                            profesor = st.selectbox("Profesor", lista_profesores, key=f"prof_{key}", index=-1)
